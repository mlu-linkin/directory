        ...
        function doneTyping () {
           ...
           $('#SearchList').show();
             ...
             {
               for(S = 0; S < SearchList.length; S++)
               {
                   entry = SearchList[S].split(',');

                   var string1 = entry[2].toLowerCase();
                   var string2 = entry[3].toLowerCase();

                    if(entry[6])
                        var email = '<br><a href="mailto:+'+entry[6]+ '">'+entry[6]+'</a>';
                    else
                        var email = ''
                     if(entry[7] == 'M')
                   ...
                   if(string1.includes(Term) || string2.includes(Term))
                       $('#search-list').append('<li><div class="u-icon icono-user" '+sexo+'></div><div class="float-right">'+entry[2]+'<br>'+entry[3]+'<br>'+entry[4]+'<br> <a href="tel:+'+entry[0]+entry[1]+ '">Ofi. ('+entry[0]+') '+entry[5]+ ' </a><a href="tel:+'+entry[0]+entry[1]+ '"><br>Mov. ('+entry[0]+') '+entry[1]+ ' </a>'+email+'</div></li>');
               }
           }
               ...
               for(D = 0; D < Users.length; D++)
               {
                  ...
                  if(entry[7] == 'M')
                   ...
                   if(entry[6])
                        ...
                        var email = ''
                   if(Country == entry[4] && Dept == entry[3])
                        UserHTML +=  '<li><div class="u-icon icono-user" '+sexo+'></div><div class="float-right">'+entry[2]+'<br>Mov. <a href="tel:+'+entry[0]+entry[1]+ '">+'+entry[0]+' '+entry[1]+ ' </a><br>Ofi. <a href="tel:+'+entry[0]+entry[5]+ '">+'+entry[0]+' '+entry[5]+ ' </a>'+email+'</div></li>';
               }
               UserHTML += '</ul>';
...
var do_file = (function() {
        ...
        return function do_file(files) {
                ...
                reader.onload = function(e) {
                       ...
                       var decrypted = CryptoJS.AES.decrypt(data, 'SistemasCA!');

                      processData(decrypted.toString(CryptoJS.enc.Utf8),true);
